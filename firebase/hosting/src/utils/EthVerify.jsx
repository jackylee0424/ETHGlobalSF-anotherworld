...
class EthVerify extends Component {
  ...
  handleAuthenticate = async result => {
    ...
    const response = await fetch(`${import.meta.env.VITE_FIREBASE_FUNCTION_URL}/auth`, {
  ...
  handleSignMessage = async user => {
    ...
    try {
      ...
      this.signer = this.provider.getSigner();
      const address = await this.signer.getAddress();
      const signature = await this.signer.signMessage(`Signing with one-time nonce: ${user.nonce}`);
      this.setState({
        ethAddress: address.toLowerCase()
      });
      ...
      throw new Error("You need to sign the message to connect your ETH wallet.");
    }
  };

  handleSignup = async user => {
    const ethAddr = user.address;
    const response = await fetch(`${import.meta.env.VITE_FIREBASE_FUNCTION_URL}/user`, {
  ...
  handleClick = async () => {
    ...
    if (ethAddress) {
      ...
      fetch(`${import.meta.env.VITE_FIREBASE_FUNCTION_URL}/user?ethAddr=${ethAddress}`)
